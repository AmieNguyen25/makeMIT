...
# ElevenLabs client with better error handling
try:
    ...
    if elevenlabs_api_key and len(elevenlabs_api_key) > 10:
        ...
        client = None
        print("⚠️  ElevenLabs API key not found or invalid - TTS will use browser fallback")
...
def text_to_speech():
    """Enhanced ElevenLabs TTS endpoint with better error handling"""
    try:
        ...
        # Check if API key is valid and client is initialized
        api_key = os.getenv("ELEVENLABS_API_KEY")
        if not api_key or len(api_key) < 10 or not client:
            print("ElevenLabs API key missing or invalid, or client not initialized")
            return jsonify({
                "error": "ElevenLabs API key not configured or client initialization failed",
                "audio": None,
                "use_browser_tts": True
            }), 200
        ...
        try:
            audio_stream = client.text_to_speech.convert(
                ...
                model_id="eleven_turbo_v2_5",  # Faster, more reliable model
                text=text,
                voice_settings={
                    "stability": 0.65,  # Consistent stable tone
                    "similarity_boost": 0.75,  # Consistent voice character
                    "style": 0.2,  # Subtle natural expression
            ...
            print(f"✅ ElevenLabs TTS successful: {len(audio_bytes)} bytes")
            api_stats['elevenlabs_calls'] += 1

            return jsonify({
                "audio": audio_base64,
                "message": text,
                "voice_info": "Bella (ElevenLabs)"
            })
            
        except Exception as tts_error:
            print(f"ElevenLabs TTS generation error: {tts_error}")
            raise tts_error
        ...
        return jsonify({
            "error": f"ElevenLabs TTS unavailable: {str(e)}",
            "audio": None,
            "use_browser_tts": True,
            "fallback_message": data.get('text', 'Thanks for recycling!') if 'data' in locals() else 'Thanks for recycling!'
        }), 200
...
@app.route("/fast-thankyou-speech", methods=["POST"])
def fast_thankyou_speech():
    """Ultra-optimized endpoint with aggressive caching and minimal API calls"""
    start_time = time.time()
    
    try:
        # Prioritize cache first (95% hit rate target)
        with cache_lock:
            if message_cache:
                cached_message = message_cache.popleft()
                api_stats['cache_hits'] += 1
                print(f"Using cached message: {cached_message}")
                text = cached_message
            else:
                # Use smart fallback selection based on time/usage patterns
                api_stats['fallback_uses'] += 1
                text = random.choice(fallback_messages)
                print(f"Using smart fallback: {text}")
        
        # Generate TTS with optimized settings and better error handling
        api_stats['elevenlabs_calls'] += 1
        api_key = os.getenv("ELEVENLABS_API_KEY")
        if not api_key or len(api_key) < 10 or not client:
            print("ElevenLabs API key missing or client not initialized")
            raise Exception("ElevenLabs TTS not available")

        print(f"Generating fast TTS for: {text[:30]}...")
        ...
        try:
            ...
            audio_base64 = base64.b64encode(audio_bytes).decode("utf-8")
            print(f"✅ Fast TTS successful: {len(audio_bytes)} bytes")
            
        except Exception as tts_error:
            print(f"ElevenLabs TTS failed: {tts_error}")
            raise tts_error
        
        # Smart cache refilling (only if critically low AND not recently refilled)
        with cache_lock:
            cache_critically_low = len(message_cache) < 2
        
        if cache_critically_low:
            # Background refill with rate limiting
            threading.Thread(target=smart_cache_refill, daemon=True).start()
        
        processing_time = (time.time() - start_time) * 1000
        print(f"Fast speech generated in {processing_time:.0f}ms")
        
        return jsonify({
            "message": text,
            "audio": audio_base64,
            "processing_time_ms": round(processing_time),
            "source": "cached" if len(message_cache) > 0 else "fallback",
            "cache_status": f"messages:{len(message_cache)}, greetings:{len(greeting_cache)}"
        })
        
    except Exception as e:
        print(f"Fast TTS Error: {e}")
        # Ultra-fast fallback with browser TTS
...
# Optimized cache initialization with minimal API usage
def initialize_cache():
    """Initialize caches with mostly fallbacks and minimal AI generation"""
    # Pre-populate with fallbacks first (no API calls)
    with cache_lock:
        # Add some fallback messages to cache immediately
        initial_messages = random.sample(fallback_messages, min(8, len(fallback_messages)))
        for msg in initial_messages:
            message_cache.append(msg)
        
        # Add some fallback greetings
        initial_greetings = random.sample(fallback_greetings, min(5, len(fallback_greetings)))
        for greeting in initial_greetings:
            greeting_cache.append(greeting)
    
    print(f"Cache initialized with {len(message_cache)} messages and {len(greeting_cache)} greetings")
    
    # Only generate AI content if specifically enabled AND after delay
    def delayed_ai_cache():
        time.sleep(10)  # Wait 10 seconds before any AI calls
        smart_cache_refill()  # This will only generate if cache is low
    
    if os.getenv('ENABLE_AI_CACHE_INIT', 'false').lower() == 'true':
        threading.Thread(target=delayed_ai_cache, daemon=True).start()

# Initialize cache with delayed, minimal AI usage
threading.Timer(1.0, initialize_cache).start()
